<template>
  <div ref="dropdownContainerRef" class="relative">
    ...
    <div class="absolute right-2 top-1/2 -translate-y-1/2">
      <button class="bg-[#9370DB] text-white rounded-full text-sm px-3 py-1 cursor-pointer" @click.stop="handleSearch">
        确认
      </button>
    </div>
    <div
...
import { searchCardApi } from '@/api/card'

const props = defineProps({
  searchType: Number, // 1. 首页下拉搜索 2. 卡片搜索
})
...
const contentList = computed(() => {
  return searchRes.value.contentList
})
const userList = computed(() => {
  return searchRes.value.userList
})
